...
class Model():
    def __init__(self, model_template, num_options, ob_space, ac_space, nenvs, nsteps, nstack, num_procs,
        ...
        self.responsible_opt_q_vals = tf.gather_nd(params=self.train_model.q_values_options,
                                                   indices=self.responsible_options)  # Extract q values for each option
        self.disconnected_q_vals_option = tf.gather_nd(params=self.disconnected_q_vals,
        ...
        self.value = tf.reduce_max(self.train_model.q_values_options) * (1 - option_eps) + (
        ...
        self.value_loss = vf_coef * tf.reduce_mean(
            vf_coef * 0.5 * tf.square(self.rewards - self.responsible_opt_q_vals))
        ...
        self.termination_loss = tf.reduce_mean(
            self.terminations * ((self.disconnected_q_vals_option - disconnected_value) + self.deliberation_costs))
...
def learn(model_template, env, seed, nsteps=5, nstack=4, total_timesteps=int(80e6), args=None):
    ...
    model = Model(model_template=model_template, num_options=num_options, ob_space=ob_space, ac_space=ac_space,
                  nenvs=nenvs, nsteps=nsteps, nstack=nstack, num_procs=num_procs, ent_coef=ent_coef, vf_coef=vf_coef,
                  max_grad_norm=max_grad_norm, lr=lr, alpha=alpha, epsilon=epsilon, total_timesteps=total_timesteps,
                  lrschedule=lrschedule, option_eps=option_eps, delib_cost=delib_cost)
