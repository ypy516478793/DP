...
def output_q_tables():
    ...
    q_table = pd.DataFrame(index=list(range(N_F)), columns=list(range(sum(N_O))), dtype=np.float64)
    q_table.iloc[:,:] = critic.sess.run(critic.v, feed_dict={critic.s: all_state})
    q_table['action'] = q_table.idxmax(axis=1)
    print("")
    print(" q_table:")
    print(q_table.sort_index(axis=0, ascending=True))
        ...
        while not option_done_0 and not done:
            ...
            while not option_done_1 and not done:
                ...
                if done:
                    ...
                    U = (1 - beta1) * hierarchical_Q_next[0][actor.chosen_options[0]] + beta1 * np.max(
                        hierarchical_Q_next[0])
